...
def standard_image_transform_3d(n_x, n_y, n_z, s_x, s_y, s_z, device=None):
...
def standard_image_transform_2d(n_x, n_y, s_x, s_y):

    # 2) Construct an affine transform M, b for (i,j)->(x,y).
    #    Let's set the center of the volume at i_mid, j_mid, 
    #    and voxel size in x,y directions = voxel_size.
    i_mid = (n_x - 1) / 2.0
    j_mid = (n_y - 1) / 2.0
    ...
    # We want the center (i_mid, j_mid, k_mid) to map to (0,0,0).
    # So we set b accordingly:
    b = torch.tensor([-i_mid * s_x, -j_mid * s_y],
                        dtype=torch.float32)
